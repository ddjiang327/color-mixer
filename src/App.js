...
const BASE_COLORS = [
  { id: "red",    label: "红", rgb: [220, 38, 38],   hex: "#DC2626" },
  { id: "yellow", label: "黄", rgb: [234, 179, 8],   hex: "#EAB308" },
  { id: "blue",   label: "蓝", rgb: [37, 99, 235],   hex: "#2563EB" },
  { id: "black",  label: "黑", rgb: [15, 15, 15],    hex: "#0F0F0F" },
  { id: "white",  label: "白", rgb: [245, 245, 245], hex: "#F5F5F5" },
];

const SECONDARY_COLORS = [
  { id: "orange", label: "橙", from: ["red","yellow"], fromLabel: "红＋黄", hex: "#F97316", rgb: [249, 115, 22] },
  { id: "green",  label: "绿", from: ["blue","yellow"], fromLabel: "蓝＋黄", hex: "#22C55E", rgb: [34, 197, 94] },
  { id: "purple", label: "紫", from: ["red","blue"],   fromLabel: "红＋蓝", hex: "#A855F7", rgb: [168, 85, 247] },
];
...
const SWATCH_CATEGORIES = [
  {
    name: "大地·自然",
    swatches: [
      { name: "象牙白",  hex: "#FFFFF0" }, { name: "奶油",    hex: "#FFFDD0" },
      { name: "米白",    hex: "#FAF0E6" }, { name: "沙丘",    hex: "#E8D5B7" },
      { name: "燕麦",   hex: "#D4B896" }, { name: "驼色",    hex: "#C19A6B" },
      { name: "焦糖",   hex: "#A0522D" }, { name: "巧克力",  hex: "#7B3F00" },
      { name: "深棕",   hex: "#4A2C0A" }, { name: "墨炭",    hex: "#2C2C2C" },
      { name: "苔藓",   hex: "#8A9A5B" }, { name: "橄榄",    hex: "#6B7C3A" },
    ]
  },
  {
    name: "红·粉·橙",
    swatches: [
      { name: "婴儿粉",  hex: "#FFD1DC" }, { name: "玫瑰粉",  hex: "#F783AC" },
      { name: "樱花",   hex: "#FFB7C5" }, { name: "珊瑚",   hex: "#FF6B6B" },
      { name: "桃红",   hex: "#FF4D6D" }, { name: "胭脂",   hex: "#E0003C" },
      { name: "朱砂",   hex: "#C0392B" }, { name: "砖红",   hex: "#C92A2A" },
      { name: "柿子橙", hex: "#FF6929" }, { name: "杏橙",   hex: "#FF922B" },
      { name: "蜜橘",   hex: "#FFA94D" }, { name: "琥珀",   hex: "#E67E22" },
    ]
  },
  {
    name: "黄·金·绿",
    swatches: [
      { name: "香槟",   hex: "#FAF0BE" }, { name: "柠檬",   hex: "#FFF44F" },
      { name: "金菊",   hex: "#FAB005" }, { name: "向日葵", hex: "#FFC300" },
      { name: "黄绿",   hex: "#ADFF2F" }, { name: "嫩草",   hex: "#90EE90" },
      { name: "薄荷",   hex: "#51CF66" }, { name: "翡翠",   hex: "#20C997" },
      { name: "松石绿", hex: "#2EC4B6" }, { name: "深绿",   hex: "#1A7431" },
      { name: "孔雀绿", hex: "#009B77" }, { name: "军绿",   hex: "#4B5320" },
    ]
  },
  {
    name: "蓝·青·紫",
    swatches: [
      { name: "冰蓝",   hex: "#D6EAF8" }, { name: "天空蓝", hex: "#339AF0" },
      { name: "矢车菊", hex: "#6495ED" }, { name: "宝蓝",   hex: "#2563EB" },
      { name: "靛青",   hex: "#4C6EF5" }, { name: "普鲁士蓝",hex: "#003153" },
      { name: "青玉",   hex: "#0077B6" }, { name: "水鸭",   hex: "#008B8B" },
      { name: "薰衣草", hex: "#E6CCFF" }, { name: "丁香紫", hex: "#CC5DE8" },
      { name: "紫罗兰", hex: "#8B5CF6" }, { name: "深紫",   hex: "#4C1D95" },
    ]
  },
  {
    name: "中国传统色",
    swatches: [
      { name: "朱红",   hex: "#E63022" }, { name: "绛红",   hex: "#8B1A1A" },
      { name: "牡丹",   hex: "#EE4E85" }, { name: "藕荷",   hex: "#E8A0BF" },
      { name: "石榴",   hex: "#F75C2F" }, { name: "赭石",   hex: "#9C4A1A" },
      { name: "缃色",   hex: "#F0C040" }, { name: "秋香",   hex: "#8E7618" },
      { name: "葱绿",   hex: "#9ABB61" }, { name: "石绿",   hex: "#57AC45" },
      { name: "青花",   hex: "#2D5FA8" }, { name: "群青",   hex: "#3568A5" },
    ]
  },
];
...
function mixColors(counts) {
  ...
  let r=0,g=0,b=0;
  ALL_COLORS.forEach(({id,rgb})=>{ const w=(counts[id]||0)/total; r+=rgb[0]*w; g+=rgb[1]*w; b+=rgb[2]*w; });
  return [r,g,b];
}
...
export default function App() {
  ...
  const [selectedColor, setSelectedColor] = useState(null); // locked pick
  ...
  // Draw image onto canvas when eyedropper opens
  useEffect(() => {
    ...
    return (
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,flex:1,minWidth:0}}>
        <div onClick={()=>add(color.id)} style={{
          ...
          {secondary && (
            <div style={{position:"absolute",bottom:"2px",left:"50%",transform:"translateX(-50%)",fontSize:7,color:"rgba(255,255,255,0.65)",background:"rgba(0,0,0,0.55)",padding:"1px 4px",borderRadius:3,letterSpacing:"0.03em",whiteSpace:"nowrap",pointerEvents:"none"}}>{color.fromLabel}</div>
          )}
        </div>
        <div style={{fontSize:11,letterSpacing:"0.05em",color:cnt>0?"#f0ece3":"#5a5850",fontWeight:cnt>0?600:400,transition:"color 0.2s"}}>{color.label}</div>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0e0e12",color:"#f0ece3",fontFamily:"'Noto Serif SC','SimSun',serif",display:"flex",flexDirection:"column",alignItems:"center",padding:"22px 16px 50px",boxSizing:"border-box"}}>

      {/* Title */}
      <div style={{textAlign:"center",marginBottom:20}}>
        <div style={{fontSize:10,letterSpacing:"0.35em",color:"#3d3c38",marginBottom:4}}>PIGMENT STUDIO</div>
        <h1 style={{fontSize:26,fontWeight:700,margin:0,letterSpacing:"0.06em",background:"linear-gradient(135deg,#f0ece3,#c9b99a)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>dd's 调色工坊</h1>
      </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:100,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:16}} onClick={(e)=>{ if(e.target===e.currentTarget) setShowEyedropper(false); }}>
          <div style={{background:"#18181e",borderRadius:16,padding:16,maxWidth:500,width:"100%",border:"1px solid rgba(255,255,255,0.1)",maxHeight:"90vh",display:"flex",flexDirection:"column"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexShrink:0}}>
              <div>
                <div style={{fontSize:13,fontWeight:600,color:"#f0ece3",letterSpacing:"0.05em"}}>🔬 吸管取色</div>
                <div style={{fontSize:10,color:"#6b6860",marginTop:2}}>移动鼠标预览颜色，双击确认选取</div>
              </div>
            ...
            {/* Canvas container — scrollable for tall portrait images */}
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,marginTop:12,padding:"8px 12px",background:"rgba(255,255,255,0.04)",borderRadius:8}}>
              <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,flexShrink:0}}>
                ...
                <div style={{fontSize:8,color:"#4a4840"}}>悬停</div>
              </div>
              ...
              <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,flexShrink:0}}>
                ...
                <div style={{fontSize:8,color:selectedColor?"#c9b99a":"#4a4840"}}>已选</div>
              </div>
              <div style={{flex:1,minWidth:0,paddingLeft:2}}>
                <div style={{fontSize:10,color:"#6b6860"}}>{selectedColor?"点击确认使用此颜色":"双击图片锁定颜色"}</div>
              ...
              <button onClick={()=>fileRef.current.click()} style={{fontSize:10,padding:"5px 8px",background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:6,color:"#a09880",cursor:"pointer",flexShrink:0}}>换图</button>
            </div>
            {/* Confirm button */}
            <button
      ...
      <div style={{display:"flex",gap:12,marginBottom:18,width:"100%",maxWidth:500}}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",gap:7}}>
          <div style={{fontSize:10,letterSpacing:"0.2em",color:"#6b6860"}}>目标色</div>
          ...
          <div style={{display:"flex",gap:5}}>
            <button onClick={()=>fileRef.current.click()} style={{flex:1,fontSize:10,padding:"6px 0",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:6,color:"#c9b99a",cursor:"pointer",letterSpacing:"0.03em"}}>📁 导入图片</button>
            <button onClick={()=>{setShowSwatches(!showSwatches);setShowEyedropper(false);}} style={{flex:1,fontSize:10,padding:"6px 0",background:showSwatches?"rgba(201,185,154,0.15)":"rgba(255,255,255,0.05)",border:`1px solid ${showSwatches?"rgba(201,185,154,0.4)":"rgba(255,255,255,0.1)"}`,borderRadius:6,color:"#c9b99a",cursor:"pointer"}}>🎨 色卡</button>
          </div>
          {uploadedImage && (
            <button onClick={()=>setShowEyedropper(true)} style={{fontSize:10,padding:"6px 0",background:"rgba(255,200,100,0.1)",border:"1px solid rgba(255,200,100,0.3)",borderRadius:6,color:"#fbbf24",cursor:"pointer",letterSpacing:"0.05em"}}>🔬 吸管取色</button>
          )}
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",width:92}}>
          <div style={{position:"relative",width:92,height:92}}>
            ...
            <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
              ...
              <div style={{fontSize:9,color:"#6b6860",letterSpacing:"0.1em",marginTop:2}}>相似度</div>
            </div>
          </div>
        </div>
        ...
        {/* Mixed */}
        <div style={{flex:1,display:"flex",flexDirection:"column",gap:7}}>
          <div style={{fontSize:10,letterSpacing:"0.2em",color:"#6b6860"}}>调出色</div>
          <div style={{height:92,borderRadius:12,background:total===0?"rgba(255,255,255,0.04)":mixedHex,border:"1px solid rgba(255,255,255,0.08)",boxShadow:total===0?"none":`0 8px 28px ${mixedHex}50`,display:"flex",alignItems:"center",justifyContent:"center",transition:"background 0.4s,box-shadow 0.4s",position:"relative",overflow:"hidden"}}>
            {total===0&&<span style={{fontSize:10,color:"#6b6860"}}>待调色</span>}
          ...
          <button onClick={reset} style={{fontSize:10,padding:"6px 0",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:6,color:"#c9b99a",cursor:"pointer"}}>↺ 重置</button>
        </div>
      </div>
            ...
            {SWATCH_CATEGORIES[activeCat].swatches.map(s=>(
              ...
              >
                <div style={{
                  ...
                  onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}
                />
                <div style={{fontSize:9,color:"#5a5850",letterSpacing:"0.02em",textAlign:"center",lineHeight:1.2,maxWidth:"100%",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.name}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {total>0 && (
        <div style={{width:"100%",maxWidth:500,background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,padding:"12px 14px",marginBottom:18,display:"flex",flexDirection:"column",gap:7}}>
          <div style={{fontSize:10,letterSpacing:"0.2em",color:"#6b6860",marginBottom:2}}>调色比例</div>
          {ALL_COLORS.filter(c=>counts[c.id]>0).map(c=>(
            <div key={c.id} style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <div style={{fontSize:11,color:"#a09880",width:16}}>{c.label}</div>
      ...
      <div style={{width:"100%",maxWidth:500}}>
        <div style={{fontSize:10,letterSpacing:"0.25em",color:"#3d3c38",marginBottom:10}}>基 色</div>
      ...
      <div style={{width:"100%",maxWidth:500,display:"flex",alignItems:"center",gap:10,margin:"18px 0 14px"}}>
        ...
        <div style={{fontSize:10,letterSpacing:"0.2em",color:"#4a4840",whiteSpace:"nowrap"}}>间 色（混合生色）</div>
        <div style={{flex:1,height:1,background:"rgba(255,255,255,0.07)"}}/>
      </div>
